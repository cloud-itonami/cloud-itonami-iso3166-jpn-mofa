...
2. Register the client's intake: business type, the specific
   MOFA-regulated activity involved, prior filing/compliance
   history in Japan if any.
3. Run the advisor in read-only mode against Ministry of Foreign Affairs's
   (外務省) published guidance.
...
5. Enable gated filing/compliance-draft assistance once the
   ODA Tender-Compliance Governor contract is trusted; actual submission always
   requires human sign-off.

## Minimum Production Controls

- client-owned data store for compliance documents
- clear provenance (official MOFA source citation) for every
...
automated actions cannot bypass the ODA Tender-Compliance Governor, and a working
